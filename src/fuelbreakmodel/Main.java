	...
	public Main() {
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				...
				String test_case_description = "m_500";
				double threshold_m = 500;		// max 100% total maintenance area at 400 FT width = 22017.3913934277 acres
				...
				File input_1_file = new File(source_folder + "/model_inputs/Manuscript 22/GB_attribute_table_final_2_fires_example.txt");
//				File input_2_file = new File(source_folder + "/model_inputs/Manuscript 22/GB_attribute_table_and_fuel_breaks_core_2_fires_example.txt");
//				File input_1_file = new File(source_folder + "/model_inputs/Manuscript 22/GB_attribute_table_final.txt");
				File input_2_file = new File(source_folder + "/model_inputs/Manuscript 22/GB_attribute_table_and_fuel_breaks_core.txt");
				String output_folder = source_folder + "/model_outputs/Manuscript 22/" + test_case_description;
				...
				for (int e = 0; e < number_of_fires; e++) {
					for (int j = 0; j < number_of_PODS[e]; j++) {
						if (j != ignition_POD[e]) {	// if this is not the ignition POD
							...
							c4_ublist.add((double) 1);	// Upper bound = 1	(when X = 1 and B = 0)
							c4_num++;
						}
					}
				}
				...
				for (int e = 0; e < number_of_fires; e++) {
					for (int j = 0; j < number_of_PODS[e]; j++) {
						if (j != ignition_POD[e]) {	// if this is not the ignition POD
							...
							// Add -X[e][j]
							c5_indexlist.get(c5_num).add(X[e][j]);
							c5_valuelist.get(c5_num).add((double) -1);
							
							// add bounds
							c5_lblist.add((double) 0);						// Lower bound = 0
							c5_ublist.add((double) Double.MAX_VALUE);		// Upper bound = max
							c5_num++;
